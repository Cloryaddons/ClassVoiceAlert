...
- 修复未选择“顺劈打击”天赋时，枯萎凋零提醒仍错误按照 4 秒粘滞时间计算的问题。
- 枯萎凋零提醒现在会根据是否选择“顺劈打击”天赋自动调整粘滞持续时间。
- 修复手动右键移除枯萎凋零相关 Buff 后，本地粘滞状态没有被正确清除的问题。
- 为 Buff 移除增加轻量的一次性确认机制，避免旧的提醒计时器继续残留。

### 说明

- 本次更新不涉及白骨之盾提醒逻辑。
- 本次更新不涉及公共声音、TTS 或音源系统。
- 枯萎凋零状态监控仍采用事件驱动方式，不使用持续轮询或高频更新。
